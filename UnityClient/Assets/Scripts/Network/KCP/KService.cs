...
namespace Net
{
	...
	public sealed class KService : AService
	{
		...
		public override void Dispose()
		{
			...
			foreach (KeyValuePair<long, KChannel> keyValuePair in localConnChannels)
			{
		...
		{
			AChannel aChannel = GetChannel(id);
			if (aChannel == null)
			{
				return null;
			}
		...
		{
			if (removedChannels.Contains(id))
			{
				return null;
			}
			KChannel channel;
		...
		public override AChannel ConnectChannel(string address)
		{
			...
			return ConnectChannel(ipEndPoint2);
		}
		...
		public override void Remove(long id)
		{
			...
			removedChannels.Enqueue(id);

			// 删除channel时检查等待连接状态的字典是否要清除
			KChannel waitConnectChannel;
			if (waitConnectChannels.TryGetValue(channel.RemoteConn, out waitConnectChannel))
			{
				if (waitConnectChannel.LocalConn == channel.LocalConn)
				{
					waitConnectChannels.Remove(channel.RemoteConn);
				}
			}
		}
		...
		public override void Update()
		{
			...
			foreach (var kv in localConnChannels)
			{
				kv.Value.Update();
			}
		...
		private void TimerOut()
		{
			...
			foreach (long k in timeOutTime)
			{
				foreach (long v in timeId[k])
				{
					updateChannels.Add(v);
				}
